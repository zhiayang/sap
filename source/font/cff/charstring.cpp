...
namespace font::cff
{
	...
	struct InterpState
	{
		...
		inline void push(Operand oper)
		{
			stack.push_back(std::move(oper));
		}
	};

	static bool run_charstring(zst::byte_span instrs, std::vector<Subroutine>& global_subrs,
		std::vector<Subroutine>& local_subrs, InterpState& interp)
	{
		...
		while(instrs.size() > 0)
		{
			...
			switch(x)
			{
				...
				case CMD_RMOVETO:       interp.ensure(2); interp.in_header = false; break;
				case CMD_HMOVETO:       interp.ensure(1); interp.in_header = false; break;
				case CMD_VMOVETO:       interp.ensure(1); interp.in_header = false; break;
				...
				case CMD_ESCAPE: {
					...
					switch(x)
					{
						...
						case CMD_ESC_ADD:       interp.ensure(2); interp.pop(); clear_stack = false; break;
						case CMD_ESC_SUB:       interp.ensure(2); interp.pop(); clear_stack = false; break;
						case CMD_ESC_DIV:       interp.ensure(2); interp.pop(); clear_stack = false; break;
						case CMD_ESC_MUL:       interp.ensure(2); interp.pop(); clear_stack = false; break;
						case CMD_ESC_NEG:       interp.ensure(1); clear_stack = false; break;
						case CMD_ESC_ABS:       interp.ensure(1); clear_stack = false; break;
						case CMD_ESC_SQRT:      interp.ensure(1); clear_stack = false; break;
						case CMD_ESC_RANDOM:    interp.push(Operand().integer(0)); clear_stack = false; break;
						...
						case CMD_ESC_DROP:  interp.ensure(1); interp.pop(); clear_stack = false; break;
						case CMD_ESC_DUP:   interp.ensure(1); interp.push(interp.peek()); clear_stack = false; break;
						...
						case CMD_ESC_AND:       interp.ensure(2); interp.pop(); clear_stack = false; break;
						case CMD_ESC_OR:        interp.ensure(2); interp.pop(); clear_stack = false; break;
						case CMD_ESC_NOT:       interp.ensure(1); clear_stack = false; break;
						case CMD_ESC_EQ:        interp.ensure(2); interp.pop(); clear_stack = false; break;
						case CMD_ESC_IFELSE:
							interp.ensure(4);
							interp.pop(); interp.pop(); interp.pop();
							clear_stack = false;
